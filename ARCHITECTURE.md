...
**[Browser Subagent 토큰 절약 원칙]**
시스템 전체를 테스트하지 마십시오. 할당량(Quota) 보호를 위해, **오직 이번 태스크에서 방금 구현/수정한 특정 컴포넌트나 라우트**에 대해서만 브라우저 서브 에이전트를 파견하여 다음 항목 중 연관된 것만 검증하십시오.

1. **상호작용 검증:** 수정된 기능의 핵심 UI 동작(예: 버튼 클릭, 칸반 카드 이동, 다크모드 렌더링)이 지연 없이 작동하는가?
2. **예외 처리 검증:** 폼 검증(Validation) 오류 시 Toast 알림이 뜨거나, 권한이 없을 때 즉시 리다이렉트 되는가?
3. **산출물 제출:** 성공적으로 작동함을 증명하는 핵심 스크린샷 1~2장과 짧은 녹화물이 포함된 `Walkthrough` 아티팩트만 간결하게 생성하십시오.